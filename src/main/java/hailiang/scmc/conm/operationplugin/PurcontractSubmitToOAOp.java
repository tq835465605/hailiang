package hailiang.scmc.conm.operationplugin;
...
import hailiang.pur.bid.operationplugin.ProjectSubmitToOAOp;
...
public class PurcontractSubmitToOAOp extends AbstractOperationServicePlugIn{
	
	private static Log logger = LogFactory.getLog(PurcontractSubmitToOAOp.class);
	
	private static final String PURCONTRACT = "conm_purcontract";
	...
	private static final String workflowId = "";//流程编码
	...
	public void beforeExecuteOperationTransaction(BeforeOperationArgs e) {
		...
		for(DynamicObject obj : dynameobjs){
			try {
				DynamicObject bidproject=BusinessDataServiceHelper.loadSingle(obj.get(CommonConstant.ID),PURCONTRACT);
				...
				if(result_json==null || !result_json.getBooleanValue("operateResult")) {
					e.setCancel(true);
					e.setCancelMessage("合同流程创建失败:"+result_json.getString("operateMsg"));
				}
				...
				e.setCancel(true);
				e.setCancelMessage("合同流程创建失败:"+e1.getMessage());
			}
		}
		//e.setCancel(true);
	}
