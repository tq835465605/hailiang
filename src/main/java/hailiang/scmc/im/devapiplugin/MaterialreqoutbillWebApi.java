...
public class MaterialreqoutbillWebApi  implements IBillWebApiPlugin{
	...
	//默认的库存事务编码
	private static final String INVSCHEME_DEFAULT = "320";
	//默认币别
	...
	private static final String BIZDEPT = "bizdept";
	...
	public ApiResult doCustomService(Map<String, Object> params) {
		...
		try {
			...
			DynamicObject materialreqbill = BusinessDataServiceHelper.newDynamicObject(im_materialreqoutbill_ext);
			//相当于where条件
			...
			QFilter invschemefilter = new QFilter(CommonConstant.NUMBER, QCP.equals, INVSCHEME_DEFAULT);
			DynamicObject adminInvscheme = BusinessDataServiceHelper.loadSingle(CommonConstant.IM_INVSCHEME, CommonConstant.ID, new QFilter[] {invschemefilter});
			adminInvscheme = BusinessDataServiceHelper.loadSingle(adminInvscheme.getLong(CommonConstant.ID),CommonConstant.IM_INVSCHEME);
			materialreqbill.set(INVSCHEME, adminInvscheme);
			...
			materialreqbill.set(BIZDEPT, adminDept);
			...
			materialreqbill.set(REQUSER, adminUser);
			...
			if(entryArray!=null) {
				...
				for (int i = 0; i < entryArray.size(); i++) {
					...
					DynamicObject adminMaterialLinventoryInfo = BusinessDataServiceHelper.loadSingle(adminMaterial.getString(CommonConstant.ID), CommonConstant.BD_MATERIALLINVENTORYINFO);
					...
					//默认仓库001
					DynamicObject adminWarehouse = BusinessDataServiceHelper.loadSingle(CommonConstant.BD_WAREHOUSE, CommonConstant.ID, new QFilter[] {new QFilter(CommonConstant.NUMBER,QCP.equals, "001")});
					billentry.set("warehouse", adminWarehouse);
					//出库库存类型，默认110=普通
					//DynamicObject adminOutinvtype = BusinessDataServiceHelper.loadSingle("bd_invtype", CommonConstant.ID, new QFilter[] {new QFilter(CommonConstant.NUMBER,QCP.equals, "110")});
					DynamicObjectCollection collection = adminInvscheme.getDynamicObjectCollection("outinvtypeentry");
					DynamicObject outinvtype = null;
					DynamicObject outinvtype_default = null;
					for(DynamicObject dynamicObject:collection) {
						outinvtype_default = dynamicObject.getDynamicObject("outinvtype");
						boolean isdefault = dynamicObject.getBoolean("outinvtypeisdefault");
						if(isdefault) {
							outinvtype = dynamicObject.getDynamicObject("outinvtype");
						}
					}
					if(outinvtype!=null)
						billentry.set("outinvtype", outinvtype);
					else
						billentry.set("outinvtype", outinvtype_default);
					//库存状态，110=可用
					//adminInvscheme.getDynamicObject("bd_invstatus");
					//DynamicObject adminOutinvsta = BusinessDataServiceHelper.loadSingle("bd_invstatus", CommonConstant.ID, new QFilter[] {new QFilter(CommonConstant.NUMBER,QCP.equals, "110")});
					//adminOutinvsta = adminInvscheme.getDynamicObject("outinvstatus");
					billentry.set("outinvstatus", adminInvscheme.getDynamicObject("outinvstatus"));
//					DynamicObject adminOutownertype = BusinessDataServiceHelper.loadSingle("bd_outownertype", CommonConstant.ID, new QFilter[] {new QFilter(CommonConstant.NUMBER,QCP.equals, "110")});
					billentry.set("outownertype", adminInvscheme.getString("outownertype"));
					billentry.set("outowner", adminorg);
					billentry.set("outkeepertype",adminInvscheme.getString("outkeepertype"));
			...
			if ((Boolean)materialreqbillSaveMap.get("result")) {
				...
				DynamicObject newMaterialreqbill = BusinessDataServiceHelper.loadSingle(materialreqbillID,im_materialreqoutbill_ext);
				OperationResult materialreqbillSubmitResult = OperationServiceHelper.executeOperate(CommonConstant.SUBMIT, im_materialreqoutbill_ext, new DynamicObject[]{newMaterialreqbill}, option);
				...
				if((Boolean)materialreqbillSubmitMap.get("result"))
				{
					...
					OperationResult materialreqbillAuditResult = OperationServiceHelper.executeOperate(CommonConstant.AUDIT, im_materialreqoutbill_ext, new DynamicObject[]{subMaterialreqbill}, option);
